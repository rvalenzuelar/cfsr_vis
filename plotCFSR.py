...
class create(object):
	...
	def isotac(self):
		...
		for i in range(6):
			...
			cs = self.axes[i].contourf(X,Y,SPD,range(40,70,5))
			self.axes[i].set_aspect(0.5)
...
def read_files(self,var):
	...
	for d in self.dates:
		...
		sub2=sub[gindx[2]:gindx[3],gindx[0]:gindx[1]]
		array.append(sub2)
